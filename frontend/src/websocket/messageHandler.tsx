...
export function useMessageHandler({queryClient}: {queryClient: QueryClient}) {
  function onMessage(data: ServerMessage) {
    if (data.type === "itemUpdated") {
      ...
      void queryClient.refetchQueries({queryKey: ["item", listId, itemId]});
    } else if (data.type === "itemAdded") {
      const {listId} = data;
      void queryClient.refetchQueries({queryKey: ["listItems", listId]});
    } else if (data.type === "listUpdated") {
      const {listId} = data;
      void queryClient.refetchQueries({queryKey: ["list", listId]});
    }
  }
